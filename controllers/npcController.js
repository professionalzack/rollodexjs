      ...
      case "get":
        controller.LookupNPC(args.join(" "), (err, nonpc) => {
          if (err) {
            ...
            let npc = nonpc[0] || {
              faction: "Pretender's Guild",
              description:
                "An absolute **imposter** who is not on the roster!!",
              name: args.join(" ")
            };
            message.channel.send(
              `${npc.name} - ${npc.faction.join(", ")}\n  *${npc.description}*`
            );
          }
        });
      ...
      case "describe":
        ...
        controller.describeNPC(name, desc, (err, npc) => {
          if (err) {
            console.log("if else!");
            message.channel.send(
              "it's not polite to talk about somebody when they don't exist"
            );
          } else {
            if (npc) {
